...
"""
    try:
        ...
        lifestyle_label = {"campus": "캠퍼스", "office": "오피스", "daily": "데일리", "freelance": "프리랜서"}.get(lifestyle, lifestyle)
        mood_label = {"minimal": "미니멀", "casual": "캐주얼", "cityboy": "시티보이", "amekaji": "아메카지", "sports": "스포츠", "vintage": "빈티지"}.get(style_mood, style_mood)
        return f"{lifestyle_label} · {budget_label} · {mood_label}"
...
async def recommend_first_item(profile: StyleProfile) -> dict:
    budget_label = {
        ...
        "10_to_20": "10~20만원",
        "over_20": "20만원 이상",
    }.get(profile.budget_range, profile.budget_range)
...
"""
    try:
        ...
        return {
            ...
            "brand": "무신사 스탠다드",
            "price": 29000,
            "reason": "미니멀 캐주얼의 핵심 아이템. 가진 하의 모두와 바로 연결됩니다.",
            "combinations": [
                {"label": "조합 1", "description": "화이트 셔츠 + 청바지 + 흰 스니커즈"},
                {"label": "조합 2", "description": "화이트 셔츠 + 카고 팬츠 + 뉴발란스"},
                {"label": "조합 3", "description": "화이트 셔츠 + 슬랙스 + 로퍼"},
